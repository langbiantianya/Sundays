...
object RequestDispatcher {
    ...
    fun dispatch(request: Request): Flow<Response> = flow {
        ...
        // 使用 .catch{} operator 而不是 flow{} 内部 try/catch —— 避免 AbortFlowException
        // (下游取消时上游 emit 抛出的 kotlinx-coroutines 内部异常) 被错误地当作业务错误处理。
        // .catch{} operator 会自动透传 AbortFlowException，不影响直接调用方用 first()/takeWhile 等短路算子。

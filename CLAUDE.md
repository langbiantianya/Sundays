# 📖 Kotlin 数据库管理端后端架构设计文档 (V2.9)

## 1. 架构总览 (Architecture Overview)

Kotlin 后端被设计为一个**无头 (Headless)**、**无状态 (Stateless)** 的底层"数据库算力引擎"。**前端框架采用 Kotlin Multiplatform (KMP) + Compose Multiplatform（仅 Desktop 端目标：macOS / Linux / Windows）**：使用 `jvmMain` 单平台目标，Compose for Desktop (Skia) 渲染；后续若需移动端 / Web 端目标可直接扩展 `commonMain` 业务层（连接管理、调用封装、流式响应聚合），无需重写业务逻辑。**前后端均为 Kotlin** — 共享 `protobuf-kotlin-lite` DSL 生成器、`kotlinx-coroutines` + `kotlinx-serialization`，**整条工具链 Kotlin 一统**，无语言边界、无桥接开销。

**v2.9 双模式架构** — 同一份业务核心支持两种调用路径：

| 模式 | 入口 | Wire / Transport | 典型场景 |
|---|---|---|---|
| **gRPC 模式**（默认，向后兼容） | `IdbEngineServer.main(args)` → gRPC server on `:50051` (或 UDS/pipe) | gRPC HTTP/2 + Protobuf + `grpc-netty-shaded` + IPC transport SPI | 跨进程、跨语言（Go/Python 等）、远程调试、子进程隔离 |
| **Direct 直接模式** (v2.9 新增) | `IdbEngine().handle(request)` (Kotlin facade) | 同 JVM typed proto 消息直传，无任何 IPC / 序列化开销 | KMP Desktop `jvmMain` (Compose UI) 直接调用，零子进程、零 IPC、零 gRPC channel |

**Direct 模式是 v2.9 的首选集成方式** — KMP Desktop 只需 `implementation(project(":engine"))`，把引擎作为库引入，Compose ViewModel 通过 `IdbEngine().handle(request { ... })` 直接拿到 `Flow<Response>`，与 gRPC stub 的 `Flow<Response>` 语义完全一致（含 `stream`/`end` 流式分帧、envelope options `traceId`/`dryRun`/`timeoutMs`）。
...
gRPC 模式 (向后兼容 / 跨进程):
[KMP Desktop Compose UI / Go / Python / etc.] ── gRPC stub ─→ [IdbEngineImpl] ─→ [IdbEngine.handle] ─→ [RequestDispatcher] ─→ [Handlers] ─→ [Dialects] ─→ [DB]
                                                 (IPC: TCP / UDS / Named Pipe)
```

两种模式共享同一个 `RequestDispatcher`，因此 envelope options、stream frame assembly、dryRun 短路、timeout 包装等横切关注点完全一致。`IdbEngineImpl.handle` 在 v2.9 重构为薄壳 — 直接桥接 `IdbEngine.handle`，消除重复路径。

**IPC 传输抽象**（v2.1 起新增，详见 §3.6）：引擎与 KMP Desktop 前端之间的 gRPC 通信可通过 CLI 参数 `--ipc` 在三种传输间切换，**Desktop 端全覆盖**：
...
- **数据库驱动**：MySQL Connector/J 9.7.0、PostgreSQL JDBC Driver 42.7.11、H2 2.3.232、DuckDB JDBC 1.5.5.1（v2.7 新增）、**SQLite JDBC 3.46.1.3（v2.8 新增，driver 名 `Sqlite`）**
...
- **测试框架**：JUnit 5 + kotlin.test — 376 测试全量通过（0 失败 / 0 错误，1 个 IpcConfigTest 用例在 Windows-only 环境 skip），其中 H2 dialect 63 项 + DuckDB dialect 81 项 + SQLite dialect 62 项 + engine 170 项
...
// DuckDB Response: {"version":"v1.5.5","mode":"embedded",...}
// SQLite Response: {"version":"3.46.1","mode":"embedded","product":"SQLite",...}
```
...
```json
{"id":"r27","category":"SYSTEM","action":"LIST_DRIVERS","connection":{},"payload":{}}
...
| `Duckdb` | `DuckDB (Embedded OLAP)` | `EMBEDDED` | 0 | ✗ | ✗ | ✓ | `VIEWS, INDEXES, FOREIGN_KEYS, MULTI_SCHEMA, EXPORT, EMBEDDED_MODE` (无 `USERS`/`TRIGGERS`/`ROUTINES`) |
...
├── dialect-duckdb/                  DuckDB 方言插件（v2.7 新增，嵌入式 OLAP）
│   └── src/test/kotlin/com/kxxnzstdsw/dialect/DuckDBDialectTest.kt  (81 tests)
├── dialect-sqlite/                  SQLite 方言插件（v2.8 新增，嵌入式关系型）
│   └── src/test/kotlin/com/kxxnzstdsw/dialect/SQLiteDialectTest.kt   (62 tests)
│
└── engine/                          主引擎模块
    └── src/main/kotlin/com/kxxnzstdsw/
        ...
        │   ├── UserHandler.kt
        │   ├── SqlEngineHandler.kt
        │   ├── SystemHandler.kt     含 TEST_CONNECTION / SERVER_INFO / LIST_DRIVERS（v2.8）
        │   ├── ExportHandler.kt
        │   ├── ViewHandler.kt
        │   ├── IndexHandler.kt
    ...
    └── src/test/kotlin/com/kxxnzstdsw/   170 tests
        ├── ipc/
        │   ├── IpcConfigTest.kt                  (20)
        ...
        ├── integration/                          (122, 12 个 handler × H2 + DuckDB + SQLite + Envelope/Route/System/DuckDB/SQLite)
        │   ├── *HandlerIntegrationTest.kt        (60, 11 个 handler × H2)
        │   ├── DuckDBHandlerIntegrationTest.kt   (27, DuckDB 端到端)
        │   ├── SQLiteHandlerIntegrationTest.kt   (7, SQLite 端到端)
        │   ├── TypedRequestEnvelopeIntegrationTest (7, typed envelope)
        │   ├── UserGrantsIntegrationTest         (2)
        │   ├── DataGenerateIntegrationTest       (2)
        │   ├── FunctionGetDdlIntegrationTest     (2)
        │   ├── SqlExplainRouteIntegrationTest    (2)
        │   ├── EnvelopeOptionsIntegrationTest    (4)
        │   └── SystemListDriversIntegrationTest  (8, LIST_DRIVERS 元数据, v2.8 新增)
```
...
dialects/            方言插件（SPI 动态加载，5 个：idb-dialect-{mysql,postgresql,h2,duckdb,sqlite}）
```
...
### 8.3 与 KMP Desktop 前端集成（gRPC · grpc-kotlin）

> 自 v2.9 起，前端框架从 Wails v2 / v3（Go + WebView 桥）升级为 **Kotlin Multiplatform + Compose Multiplatform（仅 Desktop JVM 目标：macOS / Linux / Windows）**。`jvmMain` 落地平台特定的子进程拉起与 IPC 通道选择；业务层走 `commonMain` 共享代码（连接管理 / gRPC 客户端封装 / 流式响应聚合）。**前后端共享同一份 `.proto` + 同一份 `protobuf-kotlin-lite` DSL 生成器** — `xxxRequest { ... }` 在前端与后端语法完全一致，无任何桥接层；gRPC 客户端调用直接返回 `Flow`，Compose 端用 `collectAsState()` 直接驱动 UI。

```kotlin
// jvmMain — Desktop 端的 gRPC 客户端封装 + 引擎子进程拉起
import io.grpc.ManagedChannel
import io.grpc.netty.NettyChannelBuilder
import idb.engine.grpc.*

fun launchEngineDesktop(): ManagedChannel {
    val pb = ProcessBuilder(
        "java", "-jar", "idb-engine.jar", "--ipc", "tcp", "--port", "50051"
    ).directory(File("build/libs")).start()
    Runtime.getRuntime().addShutdownHook(Thread { pb.destroy() })
    return NettyChannelBuilder.forAddress("localhost", 50051)
        .usePlaintext()
        .build()
}

suspend fun listTables(channel: ManagedChannel) {
    val stub = IdbEngineCoroutineStub(channel)
    val req = request {
        id = "req-001"
        ...
        body = RequestBody.TableRequest(
            tableRequest = tableRequest { body = TableRequestBody.List(list = tableListRequest {}) }
        )
    }
    stub.handle(req).collect { resp ->
        if (resp.end) return@collect                              // 流式终止
        // 解析 typed per-list-item: resp.table.list.itemsList
        // 普通响应只需 .first() { !it.stream }
    }
}
```
...
| OS | 子进程拉起 | IPC 支持 | gRPC transport | UI |
|---|---|---|---|---|
| **macOS** | `java.lang.ProcessBuilder` | TCP / UDS（filesystem namespace） | grpc-netty | Compose Desktop (Skia) |
| **Linux** | `java.lang.ProcessBuilder` | TCP / UDS（filesystem namespace，epoll native） | grpc-netty | Compose Desktop (Skia) |
| **Windows** | `java.lang.ProcessBuilder` | TCP / Named Pipe（客户端） | grpc-netty | Compose Desktop (Skia) |

**流式响应**：grpc-kotlin `stub.handle(req): Flow<Response>` → `collect` 拉取 → Compose 用 `collectAsState(initial = emptyList())` 驱动列表渲染；`resp.stream` / `resp.end` 字段决定 UI 增量更新与终止。

**连接凭证缓存**：KMP `commonMain` 通过 `MultiplatformSettings`（或 JVM 端 `java.util.prefs.Preferences` / 文件 JSON）持久化连接列表；`SYSTEM.LIST_DRIVERS` 拉取的方言元数据用于动态渲染"新建连接"表单（无需为每个 driver 硬编码表单字段）。

**未来扩展**：KMP 项目结构天然支持后续扩展 Android / iOS / Web 端目标 — 只需新增 `androidMain` / `iosMain` / `wasmJsMain` source set 与平台特定的子进程拉起逻辑，`commonMain` 中的业务层（连接管理 / gRPC 封装 / 流式聚合）零修改复用。

### 8.4 与 KMP Desktop 前端集成（Direct 直接模式 · v2.9 推荐）

> 自 v2.9 起，KMP Desktop 通过 **`IdbEngine` facade 直接调用引擎方法** — 不再需要子进程、gRPC channel、IPC transport。
> Compose UI 与引擎共享同一个 JVM，typed proto 消息直传，零序列化、零桥接开销。

```kotlin
// shared/src/commonMain/kotlin/.../IdbRepository.kt
...
class IdbRepository {
    private val engine = IdbEngine()                  // 构造即 bootstrap drivers/dialects

    /** 单次调用 — 取 SYSTEM.INFO (Compose UI 启动时调用一次) */
    suspend fun systemInfo(): SystemInfoResponse =
        engine.invoke(connectionConfig { driver = "H2"; database = ":memory:" }) {
            category = Category.SYSTEM
            action = Action.INFO
            systemRequest = SystemRequest.getDefaultInstance()
        }.system.info

    /** 非流式 LIST — 取当前 schema 的所有表 */
    suspend fun listTables(schema: String): TableListResponse =
        engine.invoke(connectionConfig { driver = "Mysql"; host = "..."; port = 3306; ... }) {
            category = Category.TABLE
            action = Action.LIST
            tableRequest = TableRequest.newBuilder()
                .setList(TableListRequest.newBuilder().setSchema(schema))
                .build()
        }.table.list

    /** 流式 — 取 SQL 查询结果 (Compose 端用 collectAsState 增量渲染) */
    fun executeSql(connection: ConnectionConfig, sql: String): Flow<Response> =
        engine.handle(request {
            ...
            sqlRequest = SqlRequest.newBuilder()
                .setExecute(SqlExecuteRequest.newBuilder().setSql(sql))
                .build()
        })
}
```
...
| 维度 | Direct 直接模式 (v2.9+) | gRPC 模式 (默认) |
|---|---|---|
| 调用栈 | `Compose → IdbEngine.handle → RequestDispatcher → handler → dialect` | `Compose → gRPC stub → IPC transport → gRPC server → IdbEngine.handle → RequestDispatcher → handler → dialect` |
| Wire | typed proto messages in-JVM | gRPC HTTP/2 frames over TCP/UDS/pipe |
| 序列化 | 无 (typed objects) | Protobuf 编码 + HTTP/2 帧切分 |
| 进程模型 | 同 JVM (引擎作为 `:engine` 库的依赖) | 独立子进程 (`java -jar idb-engine.jar`) |
| 启动开销 | 0 (构造即用) | ~50ms (JVM 启动 + 驱动加载 + gRPC server bind) |
| 跨语言 | 仅 Kotlin/JVM | Go / Python / 任何 gRPC client |
| 调试 | 直接 IDE 调试 | 跨进程 attach |
| 适用场景 | KMP Desktop Compose UI（首选） | 跨进程 / 跨语言 / 远程 / 子进程隔离 |

**为什么两条路径并存**：
- **Direct 模式** = KMP Desktop 的最佳路径 — 零开销，IDE 友好，自动继承方言/驱动/连接池生命周期
- **gRPC 模式** = 保留向后兼容（v2.0 起的 wire contract 不变），同时支持跨语言客户端、子进程隔离（大数据导出）、远程调试

两条路径共享 `RequestDispatcher`，因此 envelope options (`traceId` / `dryRun` / `timeoutMs`)、stream frame assembly、`if_exists` 语义等横切关注点完全一致。`IdbEngineImpl.handle` (gRPC) 在 v2.9 重构为薄壳 — 直接桥接 `IdbEngine.handle`，消除重复路由路径。

**CLI 选项**：`java -jar idb-engine.jar --mode direct` 启动 standalone 进程，仅 bootstrap drivers/dialects 后阻塞主线程（不监听 IPC），供 shell 测试或守护进程场景使用。KMP frontend 走 library 集成，不走此 CLI 入口。
...
- 数据库方言抽象层（DatabaseDialect SPI + MySQL/PostgreSQL/H2/DuckDB/SQLite 插件，**所有 SPI 方法五个方言均完整实现**（v2.8））
- **SPI 连接元数据扩展（v2.8）**：`DatabaseDialect` 新增 `displayName` / `connectionType` / `requiresHost` / `requiresPort` / `defaultPort` / `supportsUser` / `supportsPassword` / `supportsSchema` / `supportsCrossDatabase` / `jdbcUrlExample` / `capabilities` 11 个属性（默认实现，**完全向后兼容**）；新增 `ConnectionType` 枚举（`CLIENT_SERVER` / `EMBEDDED` / `FILE_BASED` / `IN_MEMORY`）+ `DialectCapability` 枚举（`USERS` / `PRIVILEGES` / `ROUTINES` / `VIEWS` / `INDEXES` / `FOREIGN_KEYS` / `TRIGGERS` / `CROSS_DATABASE` / `MULTI_SCHEMA` / `EXPORT` / `DDL_TRANSACTION` / `EMBEDDED_MODE`）；5 个方言全部声明各自元数据
- **`SYSTEM.LIST_DRIVERS` action（v2.8 新增）**：枚举所有已加载方言并返回 `repeated DialectInfo` 元数据，供前端**动态渲染"新建连接"表单**（无需硬编码 driver/port/是否需要 user）；`RequestDispatcher` 表驱动新增 `(SYSTEM, LIST_DRIVERS) → Route` 一条；`DialectLoader.getAllDialects()` 提供枚举入口
...
- **SQLite 方言插件（v2.8 新增）**：第 5 个方言 `Sqlite`，仅本地嵌入式（`:memory:` / `.db` 文件）。host/port/user/password 全部忽略。JDBC 驱动 `org.sqlite.JDBC` 3.46.1.3。自增主键用 `INTEGER PRIMARY KEY AUTOINCREMENT`（必须 INTEGER 类型，必须 inline 在列定义里；`TableHandler.create` 检测到 `autoIncrementColumns.isNotEmpty()` 时跳过表级 `PRIMARY KEY` 子句以避免 SQLite "more than one primary key"）。FK 走 table-rebuild（CREATE temp AS SELECT → DROP → CREATE with FK → INSERT → DROP temp），`addForeignKey` 不支持 CONSTRAINT 子句名（SQLite CREATE TABLE 语法限制）。MODIFY_COLUMN 仅支持 RENAME（SQLite 无 ALTER COLUMN）。TRUNCATE 用 `DELETE FROM` + 重置 `sqlite_sequence`。多 database 用 `ATTACH/DETACH`。用户/权限/触发器/存储过程抛 `UnsupportedOperationException`。SQL 危险关键词额外禁用 `ATTACH/DETACH/PRAGMA/REPLACE/VACUUM/REINDEX`。
- **376 个测试全通过，0 失败 / 0 错误**（1 个 Windows-only IpcConfigTest 用例跳过）：
  - `:dialect-h2:test` — H2 方言 63 测试
  - `:dialect-duckdb:test` — DuckDB 方言 81 测试（v2.7 新增）
  - `:dialect-sqlite:test` — SQLite 方言 62 测试（v2.8 新增）
  - `:engine:test` — 170 测试（含 27 个 `DuckDBHandlerIntegrationTest` + 7 个 `SQLiteHandlerIntegrationTest` + 8 个 `SystemListDriversIntegrationTest`，v2.8 新增）
    - `pool/PoolManagerTest` — 11
    - `loader/DialectLoaderTest` — 5
    - `ipc/IpcConfigTest` — 20（1 个 Windows-only 跳过）
    ...
    - `integration/SystemListDriversIntegrationTest` — 8（LIST_DRIVERS 元数据枚举 + 5 个方言分别验证，v2.8 新增）

⏳ 待扩展：
- GraalVM Native Image 编译
- 更多数据库方言插件（Oracle, SQL Server — 实现 SPI 接口即可；SQLite 已于 v2.8 完成）
- 性能监控与指标上报
...
| **v2.7** | **DuckDB 方言插件（v2.7 新增）** | 新增 `dialect-duckdb` 模块（driver 名 `"Duckdb"`，JDBC `org.duckdb.DuckDBDriver` 1.5.5.1），仅本地嵌入式（内存 / `.duckdb` / `.csv` / `.parquet` / `.json` / `.xlsx`）；host/port 完全忽略，`database` 字段即路径。Excel 走 Apache POI 5.5.1 预转换为临时 DuckDB（`ExcelToDuckDbCache` 缓存避免重复转换）。自增主键用 `SEQUENCE + DEFAULT nextval + 表级 PRIMARY KEY` 兜底（因 DuckDB 1.5.5.1 不接受 IDENTITY+表级 PK 组合，也不支持 `INTEGER PRIMARY KEY` ROWID 自填充）。FK 走 table-rebuild（因 DuckDB 不支持 `ALTER TABLE ADD/DROP CONSTRAINT` + 忽略 `CONSTRAINT fk_name`，自动生成 `<table>_<cols>_fkey`）。`SHOW CREATE TABLE/VIEW` 解析失败 → 手动从 `information_schema` 重建 DDL。`duckdb_functions()` MACRO 列名 `macro_definition`（不是 `definition`/`description`）。`duckdb_constraints()` 无 `column_name`，只有 `constraint_column_names` (LIST)。FK `ON DELETE/UPDATE CASCADE/SET NULL/SET DEFAULT` 不支持 → 全部改写为 `NO ACTION`。同文件不同连接配置冲突 → 测试 fixture 强制走 `PoolManager`（不混用 DriverManager）。新 SPI 方法 `DatabaseDialect.buildPreCreateStatements(tableName, autoIncrementColumns): List<String>`（默认空实现）。299 测试全通过（81 DuckDB + 63 H2 + 155 engine） |
| **v2.8** | **SQLite 方言插件 + SPI 连接元数据扩展 + SYSTEM.LIST_DRIVERS** | 新增 `dialect-sqlite` 模块（driver 名 `"Sqlite"`，JDBC `org.sqlite.JDBC` 3.46.1.3），仅本地嵌入式（`:memory:` / `.db` 文件）；host/port/user/password 全部忽略，`database` 字段即路径。自增主键 `INTEGER PRIMARY KEY AUTOINCREMENT`（inline，**`TableHandler.create` 检测到 autoIncrementColumns 时跳过表级 `PRIMARY KEY`**）。FK 走 table-rebuild（CREATE temp AS SELECT → DROP → CREATE with FK → INSERT → DROP temp），`addForeignKey` 不支持 CONSTRAINT 子句名（SQLite 语法）。MODIFY_COLUMN 仅 RENAME。TRUNCATE 走 `DELETE FROM` + 重置 `sqlite_sequence`。多 database 走 `ATTACH/DETACH`。用户/权限/触发器/存储过程抛 `UnsupportedOperationException`。**SPI 元数据扩展**：`DatabaseDialect` 新增 `displayName` / `connectionType` / `requiresHost` / `requiresPort` / `defaultPort` / `supportsUser` / `supportsPassword` / `supportsSchema` / `supportsCrossDatabase` / `jdbcUrlExample` / `capabilities` 11 个属性（默认实现，**完全向后兼容**）。新增 `ConnectionType` 枚举（`CLIENT_SERVER` / `EMBEDDED` / `FILE_BASED` / `IN_MEMORY`）+ `DialectCapability` 枚举（`USERS` / `PRIVILEGES` / `ROUTINES` / `VIEWS` / `INDEXES` / `FOREIGN_KEYS` / `TRIGGERS` / `CROSS_DATABASE` / `MULTI_SCHEMA` / `EXPORT` / `DDL_TRANSACTION` / `EMBEDDED_MODE`）。**SYSTEM.LIST_DRIVERS action（Action 18）**：枚举所有已加载方言并返回 `repeated DialectInfo` 元数据（`driver_name` / `display_name` / `jdbc_driver_class_name` / `jdbc_url_example` / `connection_type` / `requires_host` / `requires_port` / `default_port` / `supports_user` / `supports_password` / `supports_schema` / `supports_cross_database` / `capabilities[]`），供前端**动态渲染"新建连接"表单**；`RequestDispatcher` 表驱动新增 `(SYSTEM, LIST_DRIVERS) → Route` 一条；`DialectLoader.getAllDialects()` 提供枚举入口。`DialectInfo.items` 按 `driverName` 字典序升序。**376 测试全通过（1 个 Windows-only skip），0 失败 / 0 错误**（62 SQLite + 81 DuckDB + 63 H2 + 170 engine） |
| **v2.9 (当前)** | **KMP Desktop 前端 + 双模式架构（gRPC + Direct）** | 前端栈从 Wails v2/v3（Go 主进程 + WebView2 桥）整体迁移到 **Kotlin Multiplatform + Compose Multiplatform**：KMP 工程结构（`commonMain` 业务层 + `jvmMain` 平台层），**当前仅启用 Desktop JVM 单一目标**（macOS / Linux / Windows 三端共享同一份 Compose Desktop 渲染）。**前后端均为 Kotlin** — 共享同一份 `.proto` + 同一份 `protobuf-kotlin-lite` DSL 生成器；前端 `xxxRequest { ... }` 与后端业务层 DSL 语法完全一致。**引擎 v2.9 新增 Direct 直接模式** — 新增 `com.kxxnzstdsw.engine.IdbEngine` facade 类，KMP Desktop 通过 `implementation(project(":engine"))` 把引擎作为库引入，Compose ViewModel 直接调用 `IdbEngine.handle(request { ... }): Flow<Response>` — 不启动子进程、不建立 gRPC channel、不走 IPC transport，typed proto 消息同 JVM 直传，零开销。**gRPC 模式保留向后兼容** — `IdbEngineServer.runGrpc()`（默认 `--mode grpc`）维持原有行为；`IdbEngineImpl.handle` 重构为薄壳，直接桥接 `IdbEngine.handle`，消除重复路径；`--mode direct` CLI 新增 standalone 模式（仅 bootstrap 后阻塞主线程，供 shell 测试用）。两条路径共享 `RequestDispatcher`，envelope options（`traceId`/`dryRun`/`timeoutMs`）、stream frame assembly、横切语义完全一致。`RequestDispatcher.dispatch` 重构：try/catch 移到 `.catch{}` operator（修复 `AbortFlowException` 被错误捕获导致的"Flow exception transparency violated"问题，让 direct 调用方能用 `first()`/`takeWhile` 短路算子）。新增 `IdbEngineDirectTest` 集成测试（4 个，direct 模式契约测试）。**desktopApp/build.gradle.kts** 新增 `implementation(project(":engine"))`，KMP Compose UI 与引擎共享连接池与方言生命周期。`engine/src/main/kotlin/com/kxxnzstdsw/engine/IdbEngine.kt`（新增），`engine/.../server/IdbEngineServer.kt`（新增 `--mode` CLI flag + `runGrpc`/`runDirect` 分发），`engine/.../server/IdbEngineImpl.kt`（薄壳化），`engine/.../dispatcher/RequestDispatcher.kt`（catch 移到 `.catch{}` operator），`engine/.../ipc/IpcConfig.kt`（USAGE 文本更新）。**KMP 仍支持 gRPC 子进程路径**（v2.8 行为保留）— 客户端可选用 gRPC client + 子进程拉起以获得跨进程隔离、远程调试、跨语言客户端能力。**KMP 工程结构预留后续扩展空间**（Android / iOS / Web 端只需新增 `androidMain` / `iosMain` / `wasmJsMain` source set + 平台特定拉起逻辑，`commonMain` 业务层零修改复用）。**173 engine tests 全通过，0 失败 / 0 错误**（含新增 IdbEngineDirectTest 4 项） |
